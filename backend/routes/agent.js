...
async function getPrice(symbol) {
    try {
        ...
        const res = await fetch(url, { ...fetchOptions, timeout: 5000 });
...
async function agentLoop(email) {
    ...
    try {
        ...
        // 🤖 Invoke 17-Agent Consensus Pipeline
        const aiResult = await getAIAnalysis(email, symbol, position);
        ...
        let tradeAmount = Math.min(balance * 0.01, 5);
        tradeAmount = Math.max(tradeAmount, 1);
        const quantity = tradeAmount / price;

        const stopLoss = aiResult.stopLoss || (aiResult.signal === 'BUY' ? price * 0.98 : price * 1.02);
        const takeProfit = aiResult.takeProfit || (aiResult.signal === 'BUY' ? price * 1.05 : price * 0.95);
        ...
        state.tradesToday++;
        await saveState(email, state);
        console.log(`📈 17-AGENT CONSENSUS TRADE: ${aiResult.signal} ${symbol} at ${price}, amount $${tradeAmount.toFixed(2)}`);
