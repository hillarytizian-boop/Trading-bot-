...
router.post('/test', async (req, res) => {
  ...
  try {
    ...
    res.json({ valid: true, balance: usdt ? parseFloat(usdt.free).toFixed(2) : '0.00', accountType: account.accountType });
  } catch (err) {
...
router.post('/connect', async (req, res) => {
  ...
  try {
    ...
    return res.status(400).json({ error: 'Invalid keys: ' + err.message });
  }
  const { error } = await supabase.from('users').update({ binance_api_key: apiKey.trim(), binance_secret_key: secretKey.trim() }).eq('email', email);
