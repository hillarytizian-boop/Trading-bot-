...
router.post('/connect', async (req, res) => {
  ...
  try {
    const client = Binance({ apiKey: apiKey.trim(), secretKey: secretKey.trim(), httpsAgent: agent });
...
router.get('/balance', async (req, res) => {
  ...
  try {
    const client = Binance({ apiKey: data.binance_api_key, secretKey: data.binance_secret_key, httpsAgent: agent });
