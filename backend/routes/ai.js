...
router.post('/analyze', async (req, res) => {
  const { email, market, price, indicators } = req.body;
...
Give a signal (BUY, SELL, or HOLD) with confidence and reason.
Respond ONLY JSON: {"signal":"BUY","confidence":80,"reason":"..."}`;
  ...
  try {
    ...
    if (successful.length === 0) {
      return res.json({ signal: 'HOLD', confidence: 0, reason: 'AI unavailable' });
    }
    ...
    res.json({ signal: 'HOLD', confidence: 0, reason: 'AI error: ' + error.message });
  }
});
