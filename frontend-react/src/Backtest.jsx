    ...
    <div style={{ flex: 1, overflowY: 'auto', background: DARK_BG, padding: 16 }}>
      ...
      <div style={{ background: DARK_PANEL, borderRadius: 14, padding: 16, border: `1px solid ${DARK_BORDER}`, marginBottom: 16 }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
          <div>
            ...
            <input
              ...
              style={{ width: '100%', padding: 8, background: '#0E1621', border: `1px solid ${DARK_BORDER}`, borderRadius: 8, color: TEXT }}
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={e => setParams({ ...params, riskPerTrade: parseFloat(e.target.value) })}
              style={{ width: '100%', padding: 8, background: '#0E1621', border: `1px solid ${DARK_BORDER}`, borderRadius: 8, color: TEXT }}
            />
          </div>
        </div>
        ...
        <button
          onClick={runBacktest}
          disabled={loading}
          style={{ width: '100%', marginTop: 12, padding: 10, background: TG_BLUE, color: '#fff', border: 'none', borderRadius: 8, fontWeight: 700, cursor: 'pointer', opacity: loading ? 0.6 : 1 }}
        >
      ...
      {results && (
        <div style={{ background: DARK_PANEL, borderRadius: 14, padding: 16, border: `1px solid ${DARK_BORDER}` }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 10 }}>
            <MetricCard
              label="Total Return"
              value={`${results.totalReturn?.toFixed(2) || 0}%`}
              color={results.totalReturn >= 0 ? GREEN : RED}
            />
            <MetricCard
              label="Win Rate"
              value={`${results.winRate?.toFixed(1) || 0}%`}
              color={results.winRate >= 60 ? GREEN : GOLD}
            />
            <MetricCard
              label="Max Drawdown"
              value={`${results.maxDrawdown?.toFixed(2) || 0}%`}
              color={RED}
            />
          </div>
          <MetricCard
            label="Final Balance"
            value={`$${results.finalBalance?.toFixed(2) || 0}`}
            color={GREEN}
          />
