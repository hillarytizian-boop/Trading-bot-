...
export default function Dashboard({ binance, email }) {
  ...
  if (loading) {
    return (
      <div style={{ flex: 1, display: 'flex', justifyContent: 'center', alignItems: 'center', background: DARK_BG, color: MUTED }}>
        Loading dashboard...
      </div>
    );
  }
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', background: DARK_BG, padding: 16 }}>
      ...
      {!binance?.connected && (
        <div style={{ background: 'rgba(240,180,41,0.1)', border: `1px solid ${GOLD}33`, borderRadius: 12, padding: 12, textAlign: 'center' }}>
          <span style={{ color: GOLD, fontSize: 13 }}>⚠️ Connect your Binance account to see live data</span>
        </div>
      )}
    </div>
  );
