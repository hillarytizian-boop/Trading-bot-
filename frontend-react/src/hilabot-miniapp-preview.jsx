...
function pill(color) { return { background: `${color}1f`, color, border: `1px solid ${color}55`, borderRadius: 20, padding: "9px 16px", fontSize: 13, fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap" }; }
function Dot({ delay }) { return <span style={{ width: 6, height: 6, borderRadius: "50%", background: MUTED, display: "inline-block", animation: "tgBounce 1.2s infinite", animationDelay: `${delay}s` }} />; }
function SignalChip({ signal, confidence, risk }) { const c = signal === "BUY" ? GREEN : signal === "SELL" ? RED : MUTED; return ( <div style={{ display: "flex", gap: 6, marginTop: 8, flexWrap: "wrap" }}> <span style={{ background: `${c}22`, color: c, border: `1px solid ${c}55`, borderRadius: 20, padding: "3px 10px", fontSize: 12, fontWeight: 700 }}>{signal}</span> <span style={{ background: "rgba(240,180,41,0.15)", color: GOLD, border: "1px solid rgba(240,180,41,0.35)", borderRadius: 20, padding: "3px 10px", fontSize: 12, fontWeight: 700 }}>{confidence}% conf</span> <span style={{ background: "rgba(255,255,255,0.06)", color: MUTED, border: `1px solid ${DARK_BORDER}`, borderRadius: 20, padding: "3px 10px", fontSize: 12, fontWeight: 600 }}>Risk: {risk}</span> </div> ); }
function BotBubble({ children, time }) { return ( <div style={{ display: "flex", gap: 8, marginBottom: 14, alignItems: "flex-end" }}> <div style={{ width: 28, height: 28, borderRadius: "50%", background: `linear-gradient(135deg,${TG_BLUE},${TG_BLUE_DEEP})`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, flexShrink: 0, color: "#fff", fontWeight: 700 }}>H</div> <div style={{ maxWidth: "82%" }}> <div style={{ background: DARK_BUBBLE_IN, borderRadius: "4px 16px 16px 16px", padding: "10px 14px", fontSize: 14, lineHeight: 1.5 }}>{children}</div> <p style={{ fontSize: 11, color: MUTED, marginTop: 4, marginLeft: 4 }}>{time}</p> </div> </div> ); }
function UserBubble({ children, time }) { return ( <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 14 }}> <div style={{ maxWidth: "78%" }}> <div style={{ background: `linear-gradient(135deg,${DARK_BUBBLE_OUT},#1f3a52)`, borderRadius: "16px 4px 16px 16px", padding: "10px 14px", fontSize: 14, lineHeight: 1.5, color: "#fff" }}>{children}</div> <p style={{ fontSize: 11, color: MUTED, marginTop: 4, marginRight: 4, textAlign: "right" }}>{time} <span style={{ color: TG_BLUE }}>✓✓</span></p> </div> </div> ); }
...
function MarketTag({ label, defaultSelected }) { const [sel, setSel] = useState(defaultSelected); return ( <div onClick={() => setSel(!sel)} style={{ padding: "6px 13px", borderRadius: 20, fontSize: 12, cursor: "pointer", border: sel ? `1px solid ${TG_BLUE}` : `1px solid ${DARK_BORDER}`, background: sel ? `${TG_BLUE}1f` : "rgba(255,255,255,0.02)", color: sel ? TG_BLUE : MUTED, fontWeight: 600 }}>{label}</div> ); }

function AppHeader({ onOpenSettings, binanceConnected }) { return ( <div style={{ height: 56, background: DARK_PANEL, borderBottom: `1px solid ${DARK_BORDER}`, display: "flex", alignItems: "center", padding: "0 14px", flexShrink: 0, position: "relative" }}> <div style={{ display: "flex", alignItems: "center", gap: 8, width: 90 }}> <div style={{ width: 30, height: 30, borderRadius: "50%", background: `linear-gradient(135deg,${TG_BLUE},${TG_BLUE_DEEP})`, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, color: "#fff", fontSize: 14 }}>H</div> {binanceConnected && <span style={{ width: 7, height: 7, borderRadius: "50%", background: GREEN }} />} </div> <div style={{ position: "absolute", left: 0, right: 0, textAlign: "center", pointerEvents: "none" }}> <span style={{ fontWeight: 700, fontSize: 16 }}>Hila Bot</span> </div> <div style={{ flex: 1, display: "flex", justifyContent: "flex-end" }}> <button onClick={onOpenSettings} aria-label="Settings" style={{ width: 36, height: 36, borderRadius: "50%", border: "none", background: "rgba(255,255,255,0.06)", color: TEXT, fontSize: 17, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>⚙️</button> </div> </div> ); }
...
function SignalsScreen({ binance, onOpenSettings }) {
  ...
  useEffect(() => {
    ...
    wsRef.current.onmessage = (e) => {
      ...
      if (data.p) {
        ...
        setPrice(newPrice);
        setPriceHistory(prev => { const updated = [...prev, newPrice]; return updated.slice(-50); });
      }
    };
  ...
  const runAnalysis = async () => {
    if (!price) { setMessages(prev => [...prev, { type: 'bot', time: 'now', text: '⏳ Waiting for price data...' }]); return; }
    setAnalyzing(true);
    try {
      ...
      const data = await res.json();
      setMessages(prev => [...prev, { type: 'bot', time: new Date().toLocaleTimeString(), text: 'Analysis result:', signal: data, reason: data.reason }]);
    } catch (err) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", flex: 1, minHeight: 0, background: DARK_BG, backgroundImage: "radial-gradient(circle at 30% 0%, rgba(42,171,238,0.06), transparent 45%)" }}>
      ...
      <div ref={scrollRef} style={{ flex: 1, overflowY: "auto", padding: "6px 14px 14px", minHeight: 0 }}>
        <div style={{ textAlign: "center", margin: "8px 0 16px" }}><span style={{ background: "rgba(255,255,255,0.06)", color: MUTED, fontSize: 11, padding: "4px 12px", borderRadius: 12 }}>Today</span></div>
        {messages.map((m, i) =>
          m.type === "bot" ? (
...
function TradesScreen() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/trades/active?email=${encodeURIComponent(email)}`)
      .then(r => { if (!r.ok) throw new Error('Failed to fetch active trades'); return r.json(); })
      .then(data => { setActiveTrades(data || []); setLoading(false); })
...
function HistoryScreen() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/trades?email=${encodeURIComponent(email)}`)
      .then(r => { if (!r.ok) throw new Error('Failed to fetch history'); return r.json(); })
