...
function SettingsDrawer({ open, onClose, binance, onBinanceConnect, email, selectedSymbol, onSymbolChange, paperMode, onPaperToggle }) {
  ...
  useEffect(() => {
    if (open && localEmail) {
      fetch(`${API_BASE_URL}/api/binance/status?email=${encodeURIComponent(localEmail)}`)
        .then(r => r.json())
        .then(data => {
          if (data.connected) {
            setStatus('connected');
            fetch(`${API_BASE_URL}/api/binance/balance?email=${encodeURIComponent(localEmail)}`)
              .then(r => r.json())
  ...
  const handleSymbolSelect = async (symbol) => {
    ...
    try {
      await fetch(`${API_BASE_URL}/api/user/settings`, { method: "POST", headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: localEmail || email, settings: { market: symbol.replace("/", "") } }) });
...
function SignalsScreen({ binance, onOpenSettings, selectedSymbol = "BTC/USDT", paperMode }) {
  ...
  // ─── Price data: WebSocket + polling fallback ──────────────────
  useEffect(() => {
    let ws = null;
    let pollInterval = null;
    let isActive = true;

    const updatePrice = (price, candles) => {
      if (!isActive) return;
    ...
    const fetchPriceViaPolling = async () => {
      try {
        const res = await fetch(`${API_BASE_URL}/api/ai/market-data?symbol=BTCUSDT`);
        if (!res.ok) throw new Error("Polling HTTP error");
        const data = await res.json();
        if (data && data.price) {
          updatePrice(data.price, data.candles);
        }
      } catch (e) {
    ...
    ws.onclose = () => {
      console.log("[WS] Disconnected – starting polling fallback");
      if (!pollInterval) {
        pollInterval = setInterval(fetchPriceViaPolling, 2000);
      }
    };
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      fetch(`${API_BASE_URL}/api/agent/status?email=${encodeURIComponent(CURRENT_USER.email)}`)
        .then(r => r.json())
        .then(data => {
  ...
  const startAgent = async () => {
    try {
      const res = await fetch(`${API_BASE_URL}/api/agent/start`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: CURRENT_USER.email }) });
      const data = await res.json();
      setMessages(prev => {
...
function TradesScreen() { return <div style={{ padding: 16, color: MUTED }}>Trades</div>; }
function HistoryScreen() { return <div style={{ padding: 16, color: MUTED }}>History</div>; }
...
export default function HilaBotMiniApp() {
  ...
  useEffect(() => {
    ...
    if (email) {
      fetch(`${API_BASE_URL}/api/user/settings?email=${encodeURIComponent(email)}`)
        .then(r => r.json())
        .then(data => {
          if (data.settings) {
        ...
        .catch(() => {});
      fetch(`${API_BASE_URL}/api/binance/status?email=${encodeURIComponent(email)}`)
        .then(r => r.json())
        .then(data => {
          if (data.connected) {
            setBinance(prev => ({ ...prev, connected: true }));
            fetch(`${API_BASE_URL}/api/binance/balance?email=${encodeURIComponent(email)}`)
              .then(r => r.json())
